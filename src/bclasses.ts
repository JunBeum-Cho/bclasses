...
export default class bclasses {
    ...
    async main () {
        ...
        for (let course of this.all_course_list) {
            this.my_course_list = this.my_course_list.map(function(course) { return course.replace(" ", "").toLowerCase()})
            if(this.my_course_list.includes(`${course["abbreviation"]} ${course["course_number"]}`.replace(" ", "").toLowerCase())) {
